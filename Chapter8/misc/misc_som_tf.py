...
class SOM(object):
    ...
    def __init__(self, m, n, dim, n_iterations=100, alpha=None, sigma=None):
        ...
        with self._graph.as_default():
            ...
            _alpha_op = tf.matmul(alpha, learning_rate_op)
            _sigma_op = tf.matmul(sigma, learning_rate_op)
            ...
            neighbourhood_func = tf.exp(tf.math.negative(tf.div(tf.cast(
                bmu_distance_squares, "float32"), tf.pow(_sigma_op, 2))))
            learning_rate_op = tf.matmul(_alpha_op, neighbourhood_func)
